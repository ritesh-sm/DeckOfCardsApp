...
import com.appian.exercise.Deck;

public class PokerDeck implements Deck {

    //An array of 52 cards
    private PokerCard[] deckOfCards;
     ...
     */
    public PokerDeck() {
        deckOfCards = new PokerCard[52];
        int cardCount = 0;
        for ( int suit = 0; suit <= 3; suit++ ) {
            for ( int value = 1; value <= 13; value++ ) {
                deckOfCards[cardCount] = new PokerCard(value, suit);
                cardCount++;
            }
        }
        dealCount = 0;
    ...
    public void shuffle(){
        for ( int i = deckOfCards.length-1; i > 0; i-- ) {
            ...
            deckOfCards[rand] = (PokerCard) temp;
        }
        //Reset deal count
        dealCount = 0;
    }
